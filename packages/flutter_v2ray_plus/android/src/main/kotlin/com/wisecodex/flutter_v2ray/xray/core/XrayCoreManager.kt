...
object XrayCoreManager {
    ...
    private fun injectPolicyConfiguration(config: JSONObject) {
        // Merge into existing policy rather than replacing it, so balancer
        // timeout settings (connIdle, handshake, etc.) at levels other than
        // "8" are preserved when present in the original config.
        val policy = config.optJSONObject("policy") ?: JSONObject()
        val levels = policy.optJSONObject("levels") ?: JSONObject()

        levels.put("8", JSONObject().apply {
    ...
    private fun injectRoutingRules(config: JSONObject) {
        ...
        // The API rule MUST be first. If it's appended after a catch-all balancer
        // rule ("network: tcp,udp" with no inboundTag), the stats API traffic is
        // routed to the VPN outbound instead of the internal stats handler —
        // causing statsquery to return nothing and speed to always show 0.
        val apiRule = JSONObject().apply {
            put("type", "field")
            put("inboundTag", JSONArray().put("api"))
            put("outboundTag", "api")
        }
        val newRules = JSONArray().put(apiRule)
        for (i in 0 until existingRules.length()) {
            newRules.put(existingRules.get(i))
        }
            ...
            runCatching {
                ...
                when (exception) {
                    is java.io.InterruptedIOException, is InterruptedException ->
                        Log.d(TAG, "Xray stdout monitor thread interrupted")
                    else -> Log.e(TAG, "Error in xray stdout monitor", exception)
                }
            }
        }.start()

        // stderr reader — xray logs errors and startup messages to stderr
        Thread {
            runCatching {
                xrayProcess?.errorStream?.bufferedReader()?.use { reader ->
                    reader.forEachLine { line ->
                        // Xray startup info goes to stderr; use Log.i for info-level lines,
                        // Log.e for lines containing error keywords.
                        if (line.contains("error", ignoreCase = true) ||
                            line.contains("failed", ignoreCase = true) ||
                            line.contains("panic", ignoreCase = true)) {
                            Log.e(TAG, "xray/stderr: $line")
                        } else {
                            Log.i(TAG, "xray/stderr: $line")
                        }
                    }
                }
            }.onFailure { exception ->
                when (exception) {
                    is java.io.InterruptedIOException, is InterruptedException ->
                        Log.d(TAG, "Xray stderr monitor thread interrupted")
                    else -> Log.e(TAG, "Error in xray stderr monitor", exception)
                }
            }
        }.start()
    }
