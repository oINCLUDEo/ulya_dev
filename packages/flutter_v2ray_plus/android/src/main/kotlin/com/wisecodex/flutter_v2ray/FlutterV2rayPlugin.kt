...
class FlutterV2rayPlugin : FlutterPlugin, ActivityAware, PluginRegistry.ActivityResultListener, MethodChannel.MethodCallHandler {
    ...
    private fun handleStartVless(call: MethodCall, result: MethodChannel.Result) {
        ...
        // extractServerAddress was defined but never called — fix that here.
        // It parses outbounds[0].settings.vnext[0].address from the JSON config
        // and stores it in CONNECTED_V2RAY_SERVER_ADDRESS for route exclusion.
        extractServerAddress(call, xrayConfig)
        android.util.Log.d(TAG, "handleStartVless: server=${xrayConfig.CONNECTED_V2RAY_SERVER_ADDRESS}:${xrayConfig.CONNECTED_V2RAY_SERVER_PORT}")
    ...
    private fun buildXrayConfig(call: MethodCall, config: String) = XrayConfig().apply {
        ...
        INCLUDE_SELF_IN_VPN = call.argument<Boolean>("include_self_in_vpn") ?: false
