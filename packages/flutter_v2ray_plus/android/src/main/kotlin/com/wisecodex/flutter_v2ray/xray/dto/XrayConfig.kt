...
data class XrayConfig(
    ...
    /**
     * Whether to include this app's own traffic in the VPN tunnel.
     *
     * Default = false (plugin historically excludes itself to prevent routing loops).
     * Set to true when you need the app's own network requests (e.g. purchase page API calls,
     * in-app WebViews) to be routed through the VPN tunnel.
     *
     * Safe to enable: the VPN server IP is already excluded from routes via [excludeIp],
     * and the tun2socks ↔ xray connection uses loopback (127.0.0.1), which bypasses the TUN.
     */
    var INCLUDE_SELF_IN_VPN: Boolean = false
) : Serializable
