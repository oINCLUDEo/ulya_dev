...
class FlutterV2ray {
  ...
  Future<void> startVless({
    ...
    AutoDisconnect? autoDisconnect,
    /// Android only. Routes the app's own traffic through the VPN tunnel.
    /// Enables purchase page, in-app WebViews and API calls to work while VPN
    /// is active. Safe: tun2socks uses loopback, xray server IP is route-excluded.
    bool includeSelfInVpn = false,
  }) async {
    try {
    ...
    await FlutterV2rayPlatform.instance.startVless(
      ...
      autoDisconnect: autoDisconnect,
      includeSelfInVpn: includeSelfInVpn,
    );
  }
